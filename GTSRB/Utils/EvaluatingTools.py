...
def get_activations(dataloader, model,batch_nums,batch_size,dims=2048, device='cpu'):
    ...
    """
    model.eval()
    pred_arr = np.empty((batch_nums*batch_size, dims))
...
def calculate_activation_statistics(dataloader, model,batch_nums,batch_size=64, dims=2048,
                                    device='cpu'):
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(dataloader, model, batch_nums, batch_size, dims, device)
    mu = np.mean(act, axis=0)
    sigma = np.cov(act, rowvar=False)
    return mu, sigma

def calculate_fid(raw_dataloader,gen_dataloader,model,batch_nums,batch_size, device, dims=2048):
    m1, s1 = calculate_activation_statistics(dataloader=raw_dataloader,
                                             model=model,
    ...
    m2, s2 = calculate_activation_statistics(dataloader=gen_dataloader,
                                             model=model,
                                             batch_nums=batch_nums,
                                             batch_size=batch_size,
                                             device=device,
