...
load_model=False
# 实验结果存放目录
result_dir = 'E:/Project/ZLTProgram/Images/cvae_gan_advancedV2'
...
#optimizerAUX = optim.RMSprop(aux.parameters(),lr=lr,weight_decay=0)
...
for epoch in range(stage1_Epochs):
	...
	for i, data in enumerate(trainLoader, 0):
		...
		# print("auxY:", auxY.type_as(y).squeeze(1))
		# print("y:", y)
		# print("auxY shape:", auxY.type_as(y).squeeze(1).shape)
		# print("y shape:", y.shape)
		
		# auxEncLoss = F.binary_cross_entropy(auxY.type_as(y).squeeze(1).float(), y.float())  
		# vaeLoss -= auxEncLoss
		...
		# auxLoss = F.binary_cross_entropy(auxY.type_as(y).squeeze(1).float(), y.float()) #correct order  #predY is a Nx2 use 2nd col.
		# auxLoss.requires_grad=True
